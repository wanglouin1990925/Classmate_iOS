...
protocol PostVideoTableViewCellDelegate: NSObjectProtocol {
...
class PostVideoTableViewCell: UITableViewCell {
    ...
    var delegate: PostVideoTableViewCellDelegate?
    var post: Post?
    var index: Int = -1
...
//            }
            ...
            if post.image != "" {
                storageReference.child(post.image).downloadURL { (url, error) in
                    if let error = error {
                        ...
                        self.thumbImageView.sd_setImage(with: url, placeholderImage: UIImage.init(named: "Image_post_placeholder"), options: .cacheMemoryOnly, completed: nil)
                    }
                }
    ...
    @IBAction func playButtonClicked(_ sender: Any) {
        delegate?.postCellPlayButtonClicked(self.post!)
    }
